...
import { CustomerComment, Header, Search } from "../components";
        ...
        return (<>
            {comment !== undefined && comment != null && comment instanceof Array && comment.map((item, index) => {
                return (
                    <Comment
                        actions={[(
                            <div className=" w-full flex flex-col justify-center"><span key="comment-nested-reply-to" className="w-full cursor-pointer mb-2" onClick={() => setIsOpen(item.data.ID)}>回复</span>{isOpen === item.data.ID && <CustomerComment topID={item.data.ID} setIsOpen={setIsOpen} articleId={articleId} key={"sub_comment_" + index} callBack={(data) => { let children = comments.get(item.data.ID); if (children === undefined && children === null) { children = [data] } else { children.push(data) } }} />}</div>
                        )]}
